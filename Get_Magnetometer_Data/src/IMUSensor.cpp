...
void IMUSensor::getAnglesMadgwick()
{
	...
	//yaw (z-axis ratation)
	double t3 = +2.0 *(q1 * q2 + q0 * q3);
	double t4 = (q0*q0 + q1*q1- q2*q2 - q3*q3);
	yaw.store((float)(atan2(t3, t4) * 180 / M_PI)-0.75);

	//pitch (y-axis rotation)
	double t2 = 2.0 * (q1*q3 - q0*q2);
	t2 = ((t2 > 1.0) ? 1.0 : t2);
	t2 = ((t2 < -1.0) ? -1.0 : t2);
	pitch.store(-(float)(-1*asin(t2) * 180 / M_PI));

    //roll (x-axis rotation)
    double t0 = +2.0 * (q0 * q1 + q2 * q3);
	double t1 = (q0*q0 - q1*q1 - q2*q2 + q3*q3);
    roll.store((float) (atan2(t0, t1) * 180/M_PI));
	
	/*
	double sinr = +2.0 *(q0*q1 + q2*q3);
	double cosr = +1.0 - 2.0*(q1*q1 + q2*q2);
	roll.store(atan2f(sinr, cosr)*180/M_PI);

	double sinp = +2.0 * (q0*q2 - q3*q1);
	if (fabs(sinp) >= 1)
	{
		pitch.store(-copysign(90, sinp));
	}
	else
	{
		pitch.store(-asinf(sinp) * 180 / M_PI);
	}

	double siny = 2.0 * (q0*q3 + q1*q2);
	double cosy = 1.0 - 2.0 * (q2*q2 + q3*q3);
	yaw.store(atan2f(siny, cosy) * 180 / M_PI - 0.75);
	*/
}
...
void IMUSensor::MadgwickAHRSupdate()
{
	...
	//std::cout << "Frequency: " << 1.0f / dTime << std::endl;
	_then = _now;
